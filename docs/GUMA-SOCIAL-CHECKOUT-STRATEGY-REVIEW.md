...
| Oversell prevention under concurrency | **Missing, and NOT acknowledged in code** | `packages/db/src/queries/orders.ts:209` (unlocked read: `(row.stockQty ?? 0) < quantity`) vs. line ~380 (unconditional decrement: `greatest(stockQty - qty, 0)`). Nothing between them prevents two transactions passing the same check; `greatest(…, 0)` floors at zero and so hides the oversell rather than erroring | Corrected 2026-08-20: earlier drafts wrongly cited line 250's `FOR UPDATE (deferred)` comment here — that comment governs the **coupon redemption cap**, not stock (see §5a). |
| Coupon redemption cap under concurrency | **Soft limit, explicitly acknowledged** | `packages/db/src/queries/orders.ts:243–250`, in the team's own words: "under READ COMMITTED two checkouts of the same coupon racing within the same instant can each see count = max-1 and both succeed… a hard guarantee would need a dedicated counter row locked FOR UPDATE (deferred — see review remediation)" | A real but bounded and knowingly-accepted issue. Distinct from the stock race; lower severity (a discount over-granted vs. an item sold that doesn't exist). |
| Migration/journal drift | **Confirmed, root-caused, and FIXED 2026-08-20** | `_journal.json` was missing `idx 2` despite `0002_nosy_ikaris.sql` existing on disk | **This was far more serious than first assessed.** Because drizzle-kit skips files with no journal entry, **no fresh database could be built from this repo's migrations** — proven empirically on 2026-08-20 when a clean Postgres container died at `0007` with `relation "platform_audit_log" does not exist` (a table `0002` creates). Existing databases were unaffected (0002 had run before its entry went missing), which is why it stayed hidden. Fixed by restoring the entry with 0002's original timestamp (`1783249404693`, recovered from production's own `__drizzle_migrations` row). Regression guards added in `migration-journal.test.ts`: every `.sql` file must have an entry, indices must be contiguous, timestamps must strictly increase. |
...
1. **Mission 001 — Concurrent Checkout Stock Safety** (§13): the unlocked read at `packages/db/src/queries/orders.ts:209` paired with the unconditional decrement at ~380.
...
- **Objective:** Close the gap between the unlocked read (`orders.ts:209`) and the unconditional decrement (`~380`), replacing the read-then-decrement with a single atomic conditional update per line (`UPDATE product_variants SET stock_qty = stock_qty - :qty WHERE id = :variantId AND stock_qty >= :qty RETURNING stock_qty`; roll back the whole order transaction if any line returns zero rows).
