...
export const SHOP_CATEGORY_EMOJI: Record<ShopBusinessCategory, string> = {
  ...
  "Appliance & Device Repair": "🛠️",
  "House Painting & Decorating": "🖌️",
  "Pest Control": "🐛",
...
export function emojiForShopCategory(category: string | null | undefined): string {
  ...
  return (
    ...
    (key.includes("Auto Body") || (key.includes("Car") && key.includes("Paint"))
      ? "🎨"
      : undefined) ??
    (key.includes("Auto") || key.includes("Car") || key.includes("Vehicle") ? "🔧" : undefined) ??
    (key.includes("Appliance") || key.includes("Device Repair") ? "🛠️" : undefined) ??
    (key.includes("Pest") ? "🐛" : undefined) ??
    (key.includes("Paint") && key.includes("House") ? "🖌️" : undefined) ??
    (key.includes("Wedding") ? "💍" : undefined) ??
