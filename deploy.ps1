...
# Keep CT .env + runtime uploads — never overwrite production secrets/URLs or photos.
tar -czf $tmp --exclude=node_modules --exclude=.next --exclude=.turbo --exclude=.git --exclude=simply-sweet-source --exclude=reference --exclude=.env --exclude=apps/web/public/uploads/products --exclude=apps/web/public/uploads/payment-proofs -C $repo .
