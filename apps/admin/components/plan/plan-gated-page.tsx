    ...
    <div className="relative min-h-[420px] overflow-hidden rounded-2xl border border-gray-200 bg-white">
      <div className="grid gap-3 p-6 sm:grid-cols-2" aria-hidden>
        {previewCards.map((card) => (
          <div
            key={card}
            className="rounded-xl border border-gray-100 bg-gray-50 px-4 py-8 text-sm font-medium text-gray-400"
          >
            {card}
          </div>
        ))}
      ...
      <div className="absolute inset-0 flex items-center justify-center bg-white/75 p-6 backdrop-blur-[3px]">
        <div className="max-w-sm rounded-2xl border border-gray-200 bg-white p-8 text-center shadow-xl">
          ...
          <h2 className="mt-3 text-lg font-bold text-gray-900">Unlock {title}</h2>
